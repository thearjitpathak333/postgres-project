...
async function loadCategories() {
    try {
        ...
        const dataFiles = {
            'connection-errors': 'connection-errors.json',
            'query-performance': 'query-performance.json',
            'locks-deadlocks': 'locks-deadlocks.json',
            'memory-resources': 'memory-resources.json',
            'backup-recovery': 'backup-recovery.json',
            'replication-issues': 'replication-issues.json',
            'configuration-tuning': 'configuration-tuning.json',
            'permissions-roles': 'permissions-roles.json',
            'data-corruption': 'data-corruption.json',
            'extension-errors': 'extension-errors.json',
            'index-errors': 'indexes.json',
            'indexes': 'indexes-knowledge.json',
            'joins': 'joins-knowledge.json',
        ...
        for (const [key, filename] of Object.entries(dataFiles)) {
            try {
                ...
                if (dataResponse.ok) {
                    categoryData[key] = await dataResponse.json();
                    console.log(`Data loaded for ${key}:`, categoryData[key].length, 'items');
                }
            } catch (error) {
        ...
        // Special handling for join-errors: combine joins.json and additional-join-errors.json
        try {
            const [joinsResponse, additionalJoinsResponse] = await Promise.all([
                fetch('/data/joins.json'),
                fetch('/data/additional-join-errors.json')
            ]);
            
            let joinData = [];
            if (joinsResponse.ok) {
                joinData = await joinsResponse.json();
            }
            
            if (additionalJoinsResponse.ok) {
                const additionalData = await additionalJoinsResponse.json();
                joinData = joinData.concat(additionalData);
            }
            
            categoryData['join-errors'] = joinData;
            console.log(`Combined join data loaded: ${joinData.length} items`);
        } catch (error) {
            console.warn('Could not load join data:', error);
            categoryData['join-errors'] = [];
        }
        ...
        if (contentArea) {
            contentArea.innerHTML = `
                <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">
                    ...
                    <small>Make sure you're running this from a web server (not file:// protocol)</small>
                </div>
            `;
        }
    }
                            ...
                            <div class="flex items-center flex-1">
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="flex items-center gap-4 text-sm text-medium-gray mb-1">
                                        ...
                                        ${item.description ? `<span class="flex items-center gap-1">
                                            <i class="fas fa-info-circle text-xs"></i>
                                            ${getQuickPreview(item.description)}
                                        </span>` : ''}
                                    </div>
                                </div>
                            </div>
        ...
        if (item.overview) {
            return `
                <div class="space-y-6">
                    ...
                    ${item.example_queries ? `
                    <div class="border-l-4 border-cyan-500 pl-4">
                        <h4 class="text-lg font-semibold text-dark-charcoal mb-2">Example Implementation</h4>
                        <div class="space-y-4">
                            ${item.example_queries.map(example => `
                                <div>
                                    <p class="text-sm text-medium-gray mb-2">${example.description}</p>
            ...
            return `
                <div class="space-y-6">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div class="border-l-4 border-blue-500 pl-4">
                            <h4 class="text-lg font-semibold text-dark-charcoal mb-2">Layer</h4>
                            <p class="text-medium-gray leading-relaxed font-medium">${item.layer}</p>
                        </div>
                        
                        <div class="border-l-4 border-orange-500 pl-4">
                            <h4 class="text-lg font-semibold text-dark-charcoal mb-2">Content</h4>
                            <p class="text-medium-gray leading-relaxed">${item.content}</p>
                        </div>
                    </div>
                        ...
                        <div class="space-y-3">
                            ${item.components.map(comp => `
                                <div class="bg-gray-50 p-3 rounded-lg">
                                    <h5 class="font-medium text-dark-charcoal mb-1">${comp.name}</h5>
                                    <p class="text-sm text-medium-gray">${comp.description}</p>
                                </div>
                            `).join('')}
                        </div>
                    </div>
                    ...
                    <div class="border-l-4 border-indigo-500 pl-4">
                        ...
                        <div class="diagram-container bg-white p-4 rounded-lg border border-gray-200 overflow-x-auto">
                            ${item.diagram}
                        </div>
                    </div>
                    ` : ''}
        ...
        return `
            <div class="space-y-6">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="border-l-4 border-orange-500 pl-4">
                        <h4 class="text-lg font-semibold text-dark-charcoal mb-2">Use Case</h4>
                        <p class="text-medium-gray leading-relaxed">${item.use_in_monitoring_debugging}</p>
                    </div>
                </div>
                
                <div class="border-l-4 border-green-500 pl-4">
                    <h4 class="text-lg font-semibold text-dark-charcoal mb-2">Key Columns</h4>
                    <div class="space-y-1">
                        ${item.important_columns.map(col => `
                            <div class="text-sm text-medium-gray">
                                <code class="bg-gray-100 px-2 py-1 rounded text-green-600">${col}</code>
                            </div>
                        `).join('')}
                    </div>
                </div>
...
function handleSearch(event) {
    ...
    const filteredData = data.filter(item => 
        ...
        (item.common_causes && item.common_causes.toLowerCase().includes(query))
    );
...
function getBotResponse(message) {
    ...
    if (lowerMessage.includes('error') || lowerMessage.includes('problem')) {
        return "I can help you with PostgreSQL errors! Try searching through our categories or describe the specific error you're encountering.";
    } else if (lowerMessage.includes('connection')) {
        return "For connection issues, check out our Connection & Authentication category. It covers common connection problems and authentication failures.";
    } else if (lowerMessage.includes('performance')) {
        return "Performance issues are covered in our Performance Optimization category. You'll find solutions for slow queries and tuning problems.";
    } else {
        return "I'm here to help with PostgreSQL questions! You can browse our categories or ask me about specific errors, performance, or database concepts.";
    }
}
...
function setupStatsTooltip() {
    const infoBtn = document.getElementById('stats-info-btn');
...
function getQuickPreview(description) {
